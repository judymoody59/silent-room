...
| **생성** | `harness.env` · `forge.sh` · `githooks/*` | 고치지 않는다. `harness.toml` 을 고치고 `harness render` |
| **소유** | `verify-project.sh` | 프로젝트 것이다. 갱신이 덮지 않는다 |
...
| `verify-project.sh` | **프로젝트 소유.** 계층 의존 검사·포맷·테스트 | `run-lint-test.sh` |
...
| `sync-task-issues.sh` | 원격 통합 브랜치의 분해를 읽어 **없는 task 이슈만 생성.** 머지 전이면 중단(승인 게이트). `--dry-run` | 착수 단계(분해가 있을 때만) |
